...
def fetch_and_save_user_games(u_name):
    ...
    for games_data in raw_json:
        if 'games' in games_data:
            get_raw_users = pd.json_normalize(games_data['games'])[['black.username', 'white.username']]
            new_users.update(set(get_raw_users['black.username']).union(set(get_raw_users['white.username'])))
...
def get_users_played_recursively(u_name, searched_users, executor, depth=0, max_depth=10):
    ...
    if depth < max_depth:
        ...
        with users_to_search_lock:
            users_to_search_copy = users_to_search.copy()  # Avoid modifying the set while iterating

        # Submit new tasks for each opponent recursively, but within the global thread pool
        futures = {executor.submit(get_users_played_recursively, user, searched_users, executor, depth + 1, max_depth): user for user in users_to_search_copy}
